...
export default function DashboardPage({
  ...
  const enrolledCourseIds = useMemo(
    () =>
      new Set(
        myCourses.filter((c) => c?.courseId != null).map((c) => c.courseId),
      ),
    [myCourses],
  );
  const isSelectedCourseEnrolled = useMemo(
    () =>
      ...
      enrolledCourseIds.has(Number(selectedCourseId)),
    [selectedCourseId, enrolledCourseIds],
  );
  const accountLabel = useMemo(
    () => getAccountLabel(currentUser),
    [currentUser],
  );
  ...
  useEffect(() => {
    async function loadCourseProgress() {
      ...
      const uniqueCourseIds = [
        ...new Set(
          myCourses.map((course) => Number(course.courseId)).filter(Boolean),
        ),
      ];
  ...
  function handleStartLearning() {
    if (!selectedCourseDetail?.lessons?.length) {
      notifySuccess(
        "This course has no lessons yet. Please wait for the instructor to add content.",
      );
      return;
    }
  ...
  if (activeScreen === "learning" && selectedCourseDetail) {
    return (
      <div className="dashboardRoot">
        ...
        {globalSuccess ? (
          <p className="alert success">{globalSuccess}</p>
        ) : null}
        <LearningPage
